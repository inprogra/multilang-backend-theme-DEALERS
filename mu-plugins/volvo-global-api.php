...
add_action( 'rest_api_init', function () {
    register_rest_route( 'volvo/v1', '/resolve-path', array(
        'methods'             => 'GET',
        'callback'            => 'volvo_global_resolve_canonical_path',
        'permission_callback' => '__return_true',
        'args'                => array(
            'path'   => array(
                'required'          => true,
 ...
 */
function volvo_global_resolve_canonical_path( $request ) {
    $path = $request->get_param( 'path' );
    if ( empty( $path ) ) {
        return new \WP_Error( 'no_path', 'Path not found', array( 'status' => 400 ) );
    }
    ...
    $segments = array_filter( explode( '/', $path_clean ) );
    ...
    if ( empty( $segments ) ) {
        ...
        return new \WP_REST_Response( array(
            ...
            'post_status'   => get_post_field('post_status', $page_id),
            'path_valid'    => '',
            'global'        => false,
        ), 200 );
    }
    ...
    // PAGE
    $page_by_path = volvo_global_get_page_by_path( $path_clean, array( 'page', 'campaign', 'service', 'model' ) );

    if ( $page_by_path && 'publish' === $page_by_path->post_status ) {
        restore_current_blog();
        ...
        return new \WP_REST_Response( array(
            ...
            'status'        => 200,
            'id'            => $page_by_path->ID,
            'post_type'     => $page_by_path->post_type,
            'post_status'   => $page_by_path->post_status,
            'path_valid'    => $path_clean,
            'global'        => $correct_global,
        ), 200 );
    }

    // CANONICAL PAGE
    $target_slug = end( $segments );

    // Posty, które mają podany slug
    $args = array(
        'name'        => $target_slug,
        'post_type'   => array( 'page', 'campaign', 'service', 'model' ),
        'post_status' => 'publish',
        'numberposts' => 1,
    );
    ...
    if ( empty( $found_posts ) ) {
        ...
        $args = array(
            ...
            'meta_value'  => $target_slug,
            'post_type'   => array( 'page', 'campaign', 'service', 'model' ),
            'post_status' => 'publish',
            'posts_per_page' => 1,
        );

        $old_slug_query = new \WP_Query( $args );

        if ( $old_slug_query->have_posts() ) {
            $found_posts = $old_slug_query->posts;
        } else {
            switch_to_blog(1);
    ...
    // Page found
    if ( ! empty( $found_posts ) ) {
        $matched_post = $found_posts[0];
        
        $correct_permalink = get_permalink( $matched_post->ID );
        $correct_path = str_replace( home_url(), '', $correct_permalink );
        $correct_path = trim( $correct_path, '/' );
        $correct_global = false;
        ...
        return new \WP_REST_Response( array(
            ...
            'post_status'   => $matched_post->post_status,
            'path_valid'    => $correct_path,
            'global'        => $correct_global,
        ), 200 );
    }
...
function volvo_global_get_hero_slider() {
    ...
    foreach ($slide_posts as $slide) {
        ...
        if (!empty($formatted_slide)) {
            ...
            $slides_cache_tags[] = $slide;
        }
    }

    \VGA\Classes\Cache_Tags_Collector::instance()->add(volvo_global_build_cache_tags('options-homepage', ['slide_posts' => $slides_cache_tags]));
...
function volvo_global_get_dealer_info($authenticated = false) {
    ...
    \VGA\Classes\Cache_Tags_Collector::instance()->add(volvo_global_build_cache_tags('options-dealer'));
...
function volvo_global_get_page($request) {
    ...
    \VGA\Classes\Cache_Tags_Collector::reset();
    
    \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}");
    ...
    // For homepage (path = /)
    if ($path_parts[0] === '') {
        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:home");
    ...
    // For contact pages, get page by path
        ...
        $page = volvo_global_get_page_by_path($path_parts[0]);
        
        if ($page) {
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:contact");

            $cache_site_id = $is_volvo_ms_global_page ? 1 : $blog_id;
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:page:{$page->ID}");
            ...
            */
            ...
            $response['source']             = volvo_global_get_page_source($page);
    ...
    // For models pages, get page by path
        ...
        if (count($path_parts) == 3) {
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:models");
            ...
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:page:{$response['id']}");
        }
        ...
        if (count($path_parts) == 1 || ((count($path_parts) === 3) && $path_parts[1] === 'page')) { // index
            ...
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:blog-index");
        ...
        $post = volvo_global_get_page_by_path($path_parts[0]);
        ...
        if ($post) {
            ...
            $cache_site_id = ($is_volvo_ms_global_page) ? 1 : $blog_id;
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:service");
        ...
        if (!isset($path_parts[1]) || count($path_parts) > 2) {
            ...
            $post = volvo_global_get_page_by_path($path_parts[1], 'campaign');
        ...
        if ($post) {
            ...
            $cache_site_id = ($is_volvo_ms_global_page) ? 1 : $blog_id;
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:drive-test");
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:drive-test-index");
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:page:{$post->ID}");
        }
        ...
        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:drive-test");
        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:drive-test-models");
        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:drive-test-models");
        ...
        if (!$page) {
            ...
            $cache_site_id = ($is_volvo_ms_global_page) ? 1 : $blog_id;
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:events-index");
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_site_id}:page:{$page->ID}");
        ...
        $page = volvo_global_get_page_by_path(trim($path, '/'));
        ...
        if ($page) {
            ...
            if (in_array($path_parts[0], ['szukaj', 'search'], true) && array_key_exists('search', $_GET)) {
                $search = $_GET['search'];

                $searchData = volvo_global_get_search_data($search);

                $response['page']['search'] = $searchData;
            }
    ...
    if (array_key_exists('page', $response) && is_null($response['page'])) {
        $response_resolve = volvo_global_resolve_canonical_path($request);
        $response_resolve_data = $response_resolve->get_data();
        
        if (array_key_exists('path_valid', $response_resolve_data)) {
            $path_valid = $response_resolve_data['path_valid'];
            $page = volvo_global_get_page_by_path(trim($path_valid, '/'), ['page']);
            
            if (!$page) {
                $path_valid_parts = explode('/', trim($path_valid, '/'));

                if (volvo_global_get_page_by_path(end($path_valid_parts), ['page', 'campaign', 'model'])) {
                    $response['page_valid'] = $response_resolve_data;
                }
            }
    ...
    $response['cache_tags'] = \VGA\Classes\Cache_Tags_Collector::instance()->all();
...
function volvo_global_post_page($request) {
    ...
    if (in_array($path_parts[0], ['serwis', 'service'])) {
        ...
        if ($checkYear == '1') {
            ...
            return new \WP_Error( 'invalid_request', 'Page not found.', [ 'status' => 404 ] );
        }
 ...
 * @return array
 */
function volvo_global_get_model_index(array $path, int $blog_id): array
{
    $response = [];
 ...
 * @return array
 */
function volvo_global_get_model_page_item(array $path, int $blog_id): array
{
 ...
 * @return ?array
 */
function volvo_global_get_blog_index(array $path, int $blog_id): ?array
{
    $page = get_page_by_path($path[0]);
...
function volvo_global_get_test_drive_model(string $slug, int $blog_id): array
{
    ...
    if (!empty($versions->posts)) {
        ...
        foreach ($posts as $version) {
            ...
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id_current}:page:{$version->ID}");
    ...
    if (!empty($test_car)) {
        ...
        $content = volvo_global_prepare_content_block($test_car[0]->post_content, $test_car[0]->ID, get_current_blog_id());

        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:page:{$test_car[0]->ID}");
    }
    ...
    if (volvo_global_is_multi_showroom_and_service($showroomsBlog)) {
        ...
        foreach ($showroomsIds as $id) {
            ...
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:page:{$id}");
        }
    }
 ...
 * @return array
 */
function volvo_global_get_menus(): array
{
    $menus = array();
    ...
    \VGA\Classes\Cache_Tags_Collector::instance()->add(volvo_global_build_cache_tags('menus', $locations));
 ...
 * @return array
 */
function volvo_global_format_menu_item($item): array
{
    return array(
        'id'          => $item->ID,
        'title'       => $item->title,
...
function volvo_global_get_contact_thank_you_image(): array|null
{
    ...
    if (is_null($globalFormOptions['thank-you-image'])) {
        return null;
    }
 ...
 * @return ?\WP_Post
 */
function volvo_global_get_page_by_path(string $path, string|array $type = 'page'): ?\WP_Post
{
    $page = get_page_by_path(ltrim($path), OBJECT, $type);

    if (!$page) {
        $page = volvo_global_get_global_page_by_path(ltrim($path), $type);
    }
 ...
 * @return ?\WP_Post
 */
function volvo_global_get_global_page_by_path(string $path, string|array $type = 'page'): ?\WP_Post
{
    global $is_volvo_ms_global_page;
 ...
 * @return string
 */
function volvo_global_get_page_source(object $page): string
{
...
function volvo_global_get_showrooms_employees(array $showroomsBlog): array
{
    ...
    if ( array_filter( $showrooms ) ) {
        foreach ( $showrooms as $showroom ) {
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:page:{$showroom}");
            ...
            foreach ( $employeeCategories as $category ) {
                ...
                if ( $showroomEmployees->have_posts() ) {
                    ...
                    \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:taxonomy:{$category->term_id}");
                    foreach ( $showroomEmployees->posts as $employee ) {
                        $employeeId = $employee->ID;
                        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:page:{$employeeId}");
 ...
 * @return object|bool
 */
function volvo_global_get_model_category_term(string $category_slug): object|bool
{
    switch_to_blog( 1 );
			...
			foreach ( $types as $key => $type ) {
				...
				foreach ( $models->posts as $model ) {
					...
					if ( $modelType === $key ) {
                        ...
                        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:page:{$modelId}");
					}
				}
    ...
    if ( array_filter( $variations->posts ) ) {
        if ( array_filter( $variations->posts ) ) {
            foreach($variations->posts as $p) {
                ...
                \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:page:{$variation->ID}");
...
function volvo_global_get_model_data($post, $blog_id): array|null
{
    ...
    \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:page:{$post->ID}");
    ...
    if (!$model) {
		$model = array(
			...
			'versions'  => array(),
			'content'   => $post->post_content ? volvo_global_prepare_content_block( $post->post_content, $post->ID, $blog_id ) : '', // parse blocks
		...
		foreach ( $versions->posts as $version ) {
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:1:page:{$version->ID}");
			...
			$model['versions'][] = array(
				...
				'content'                   => ($version->post_content) ? volvo_global_prepare_content_block( $version->post_content, $version->ID, $blog_id ) : '', // prepare blocks to view
...
/**
 * Model data version override
 * 
 * @param int $version_id
 * @param array $featuredCarsOptions
 * @param int $blog_id
 * @return array|null
 */
function volvo_global_get_version_override(int $version_id, array $featuredCarsOptions, int $blog_id): array
{
    switch_to_blog( $blog_id );
    ...
    foreach ( $versionOverride->posts as $override ) {
        $versionOverrideBlocks[] = ($override->post_content) ? volvo_global_prepare_content_block( $override->post_content, $override->ID, $blog_id ) : ''; // prepare blocks to view
    }
 ...
 * Format block link
 * 
 * @param array|string|null $content_link
 * @return array
 */
function volvo_global_block_link(array|string|null $content_link): array|string|null
{
 ...
 * @return array
 */
function volvo_global_get_post_tags(int $post_id): array
{
 ...
 * @return array
 */
function volvo_global_prepare_content_block(string $content, int $post_id, int $blog_id): array
{
    return volvo_global_block_prepare_to_view(volvo_global_blocks_prepare_all( $content, $post_id, $blog_id ), $post_id, $blog_id);
}
 ...
 * @return array
 */
function volvo_global_get_blog_pages(int $blog_id): array
{
    ...
    if (!empty($posts_query->posts)) {
        foreach ($posts_query->posts as $blog_post) {
            $imagesDesktop = [];

            $img_id = get_field('blog_image', $blog_post->ID);
            if (!$img_id) {
                $img_id = get_post_thumbnail_id($blog_post->ID);
            }
            ...
            $post_data = [
                'heading'       => get_the_title($blog_post->ID),
                'image'         => $imagesDesktop,
                'blog_desc'     => trim(get_field('blog_desc', $blog_post->ID)),
                'link'          => ['url' => get_permalink($blog_post->ID)],
                'date'          => get_the_date('d.m.Y', $blog_post->ID),
                'description'   => get_the_excerpt($blog_post->ID),
                //'ctaText'       => __('Read', 'partners-site_v2'), //'PRZECZYTAJ'
            ];

            if (empty($post_data['description'])) {
                $post_data['description'] = wp_trim_words(get_the_content($blog_post->ID), 30, '...');
            }
 ...
 * @return void
 */
function volvo_global_set_paged(array $page_parts = []): void
{
 ...
 * @return array
 */
function volvo_global_get_service_page_index(int $blog_id): array
{
    ...
    return array(
        ...
        'contactSection'    => volvo_global_get_service_get_contact_section($blog_id),
    );
}
...
/**
 * Dealer data on service page
 * 
 * @param int $blog_id
 * @return array
 */
function volvo_global_get_service_dealers_data(int $blog_id): array
{
    $dealersData = [];
...
function volvo_global_get_service_news(array $options, int $blog_id): array
{
    ...
    for ( $i = 0; $i < (int) $admin_news; $i++ ) {
        ...
        $news[ $i ] = array(
            ...
            'image' => ($options[ 'vinomat_news_' . $i . '_vinomat_news_image' ][0]) ? volvo_global_prepare_image($options[ 'vinomat_news_' . $i . '_vinomat_news_image' ][0]) : null, //wp_get_attachment_image_src( $options[ 'vinomat_news_' . $i . '_vinomat_news_image' ][0] )[0],
            'link'  => $url,
        );
    }
    ...
    foreach ( $dealer_news['vinomat_news'] as $n ) {
        $news[$x] = array(
            ...
            'image' => ($n['vinomat_news_image']) ? volvo_global_prepare_image($n['vinomat_news_image']) : null, // wp_get_attachment_image_src( $n['vinomat_news_image'] )[0],
 ...
 * @return array
 */
function volvo_global_get_service_form( int $blog_id, ?string $type = null ): array
{
    $showroomsBlog = volvo_global_get_showrooms_blog();
 ...
 * @return array
 */
function volvo_global_get_service_blog_ids(int $blog_id, bool $domain_only = false): array
{
 ...
 * @return array
 */
function volvo_global_get_service_get_contact_section(int $blog_id): array
{
    switch_to_blog( 1 );
...
function volvo_global_get_campaing_page(\WP_Post $post, int $blog_id): array
{
    ...
    $cache_tags_site = ($is_volvo_ms_global_page) ? 1 : $blog_id;
    \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$cache_tags_site}:page:{$post->ID}");
    ...
    $content          = volvo_global_prepare_content_block( $post->post_content, $post->ID, get_current_blog_id() );
    ...
    if (!empty($check_campaign->posts)) {
        $post_last = end( $check_campaign->posts );

        //foreach ( $check_campaign->posts as $local_camp ) {
            $content = volvo_global_prepare_content_block( $post_last->post_content, $post_last->ID, $blog_id );
        //}
        foreach ( $check_campaign->posts as $local_camp ) {
            \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:page:{$local_camp->ID}");
        }
    }
    ...
    foreach ( $campaignOverride->posts as $override ) {
        $additionalContent[] = volvo_global_prepare_content_block( $override->post_content, $override->ID, $blog_id );

        \VGA\Classes\Cache_Tags_Collector::instance()->add("site:{$blog_id}:page:{$override->ID}");
    }
 ...
 * @return array
 */
function volvo_global_get_search_data(string $search): array
{
    $results = [];
    $blog_id = get_current_blog_id();
    ...
    if ($search) {
        ...
        if (!empty($car_custom_query->posts)) {
            foreach($car_custom_query->posts as $key => $car_post) {
                $result = [
                    'ID' => get_the_ID($car_post->ID),
                    'title' => get_the_title($car_post->ID),
                    'content' => volvo_global_prepare_content_block($car_post->post_content, $car_post->ID, $blog_id),
                    'url'     => get_the_permalink($car_post->ID)
                ];
        ...
        if (!empty($search_query->posts)) {
            ...
            foreach($search_query->posts as $key => $query_post) {

                $result = [
                    'ID' => get_the_ID($query_post->ID),
                    'title' => get_the_title($query_post->ID),
                    'content' => volvo_global_prepare_content_block($query_post->post_content, $query_post->ID, $blog_id),
                    'url'     => get_the_permalink($query_post->ID)
                ];
    ...
    foreach($results as $key => $value) {
        $image_id = get_field('image', $value['ID']);

        $results[$key]['image'] = !empty($image_id) ? volvo_global_prepare_image($image_id) : '/app/themes/partners-site_v2/images/noimage.svg';
    }
