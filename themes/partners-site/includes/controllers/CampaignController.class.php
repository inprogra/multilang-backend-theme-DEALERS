...
class CampaignController extends Controller {
	...
	public function render(): string {
		...
		if ( ! $query = wp_cache_get('post-global-'.$post->ID) ) { 
...
//wp_cache_set('post-global-'.$post->ID,$check_global_campain,'',3600);
		}
		else {
			$check_global_campain = wp_cache_get('post-global-'.$post->ID);
		}
