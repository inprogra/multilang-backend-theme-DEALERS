		...
		if (array_key_exists(1, $checkCar)) {
			...
			$versions = new \WP_Query(
				array(
					...
					'post_status'	 => 'any',
					'posts_per_page' => -1,
					'title' => strtoupper(str_replace('model-','',$checkCar[1])),
			...
			if (MultisiteFixer::getCurrentBlogId() == 1) {
				...
				$data = json_decode(file_get_contents('/var/www/volvocars-partner.pl/partners-site/dealers.txt'));
			
				unset($data[0]);
				
				
				foreach($data as $a) {
					$response[] = ['key' => $a->dealerId, 'value' => $a->address[0] ,'sorter' => str_replace('Ł','L',$a->address[0])];
				}
				setlocale(LC_COLLATE, 'pl_PL.utf8');
				$arr2 = $this->array_msort($response, array('sorter'=>SORT_ASC));
				$dealersData = $arr2;
			...
			return $this->view('layouts/test-drive/test-drive-car', [
				'content_title' => $title,
				'dealerAddresses' => (MultisiteFixer::getCurrentBlogId() == 1 ? $dealersData : false),
				'content' => $content,
				'groups' => $modelsGroups,
				'source' => $source,
