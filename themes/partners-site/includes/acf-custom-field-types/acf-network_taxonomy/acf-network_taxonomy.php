...
if ( ! class_exists( 'acf_field_network_taxonomy' ) ) :

	class acf_field_network_taxonomy extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// vars
			$this->name     = 'network_taxonomy';
			$this->label    = __( 'Network Taxonomy', 'acf' );
			...
			acf_add_filter_variations( 'acf/fields/network_taxonomy/query', array( 'name', 'key' ), 1 );
			acf_add_filter_variations( 'acf/fields/network_taxonomy/result', array( 'name', 'key' ), 2 );

			// ajax
			add_action( 'wp_ajax_acf/fields/network_taxonomy/query', array( $this, 'ajax_query' ) );
			add_action( 'wp_ajax_nopriv_acf/fields/network_taxonomy/query', array( $this, 'ajax_query' ) );
			add_action( 'wp_ajax_acf/fields/network_taxonomy/add_term', array( $this, 'ajax_add_term' ) );
		...
		function input_admin_enqueue_scripts() {
			wp_register_script( 'acf-network_taxonomy', get_template_directory_uri() . '/includes/acf-custom-field-types/acf-network_taxonomy/acf-network_taxonomy.js' );
			wp_enqueue_script( 'acf-network_taxonomy' );
		}
		...
		*/
		...
		function get_ajax_query( $options = array() ) {
			...
			// filters
			$args = apply_filters( 'acf/fields/network_taxonomy/query', $args, $field, $options['post_id'] );
		...
		function get_term_title( $term, $field, $post_id = 0 ) {
			 ...
			 * @since    5.0.0
			 */
			return apply_filters( 'acf/fields/network_taxonomy/result', $title, $term, $field, $post_id );
		}
		...
		*/
		...
		function render_field_checkbox( $field ) {
			...
			$args = apply_filters( 'acf/fields/network_taxonomy/wp_list_categories', $args, $field );
			$args = apply_filters( 'acf/fields/network_taxonomy/wp_list_categories/name=' . $field['_name'], $args, $field );
			$args = apply_filters( 'acf/fields/network_taxonomy/wp_list_categories/key=' . $field['key'], $args, $field );
	...
	// initialize
	acf_register_field_type( 'acf_field_network_taxonomy' );
