...
class CarDictionary
{
	...
	public function getSettings()
	{
		...
		$tmp['global_settings'] = json_decode(file_get_contents($settings));

		return json_encode($tmp);
	}
	public function getOffer($token, $cache = true, $path)
	{
		if ($this->disable_dol) {
			return [];
		}
	...
	public function getBlogIds() {
		...
		$addresses_only = $_GET['addressess'];
		$mSalon = ['PL041','PL050'];
        ...
        foreach ($blogs as $b) {
            ...
            if ($domain_only) {
                ...
                $data = ['blog_id' => $b['blog_id'],'address'=> $addresses,'multisalon' => $multisalon, 'cars' => $count_pages,'domain'=> $b['domain'], 'dealerId' => $dealerId,'showrooms' => $showrooms,'car_ids' => $cars];
				if ($dealerId ) {
	...
	public function importCars($dataArr)
	{
		...
		$filename = date('Y-d-m-h:m');
		file_put_contents('wikicars/active-motors-'.$filename, json_encode($dataArr));
