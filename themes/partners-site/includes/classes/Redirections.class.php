...
class Redirections
{
    ...
    public function parseRequest()
    {
        ...
        $currentUrl = MultisiteFixer::getHomeUrl() . $_SERVER['REQUEST_URI'];


        if ($wp->request === 'admin' || $wp->request === 'wp-admin') {
            $this->redirect(
                array(
                    'target' => MultisiteFixer::getHomeUrl() . '/wp/wp-admin',
                    'code'   => 302,
                )
            );
        }
        $bid = MultisiteFixer::getCurrentBlogId();
        //	var_dump(parse_url($currentUrl));
        $currentUrl = parse_url($currentUrl);
        $query = '';
        if (array_key_exists('query', $currentUrl)) {
            $query = '?' . $currentUrl['query'];
        }
        $currentUrl = 'https://' . $currentUrl['host'] . $currentUrl['path'];
        $urls = $m->get('redirections');
        ...
        $redirections = $urls[$bid];

        $found = $this->find($redirections[0], $currentUrl, $query);

        if (!empty($found)) {
            $this->redirect($found);
        }

        $redirections = $urls['global'];
        $found        = $this->find($redirections, $currentUrl, $query);
    ...
    public function getDealerRedirectsCsv()
    {
        ...
        if ($redirects_admin) {
            ...
            foreach ($redirects_admin as &$Row) {
                if (strpos($Row, ',') !== false) {
                    $Row = str_replace('https://main.volvocars-partner.pl/', $domain, str_getcsv($Row, ","));
                } else {
                    $Row = str_replace('https://main.volvocars-partner.pl/', $domain, str_getcsv($Row, ";"));
                }
            }
