...
class DolStatus
{
    ...
    public static function add_to_listing($defaults, $post_type)
    {
        if ($post_type === StockCar::POST_TYPE) {
            ...
            $defaults[self::COLUMN_NAME] = 'Dol Status';
            $defaults['odreki-sync'] = 'Odreki';
            $defaults['leasing'] = 'Finansowanie';
        }
    ...
    public static function add_column_content($column_name, $post_id)
    {
        if ($column_name === self::COLUMN_NAME) {
            $dol_sync_status = get_field(self::FIELD_NAME, $post_id);
            if (!$dol_sync_status) {
                $dol_sync_status = 0;
            }
            $status = self::DOL_STATUSES[$dol_sync_status];
            $icon = self::DOL_STATUSES_ICONS[$dol_sync_status];

            echo <<<HTML
            <div aria-hidden="true" title="$status" style="max-width: 35px">$icon</div>
            HTML;
        }
        if ($column_name === 'price') { 
            $price = get_field('regular-price', $post_id);
            $prom_price = get_field('discount-price', $post_id);
            echo $price.'/'.$prom_price;
        }
        if ($column_name === 'vin') {
        ...
        if ($column_name === 'odreki-sync') {
            $odreki_sync = (int) get_field('odreki_sync',$post_id);
          
            if ($odreki_sync == 1) {
                $color = 'green';
            } else if ($odreki_sync == 2) {
                $color = 'orange';
            } else {
                $color = 'red';
            }
            ...
            echo <<<HTML
            <div class="odreki_status $color" style="width:35px;" title="Brak auta w serwisie odreki">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" version="1.1"><g id="surface1">
        ...
        if ($column_name === 'leasing') {
            $status = true;
            $lease = get_field('lease_car');
            $najem = get_field('najem-car');
            ...
            $color = (!$lease || !$status   ? 'red' : 'green');
            $message = (!$lease   ? 'Finansowanie wyłączone' : 'Finansowanie włączone');
            echo <<<HTML
            <div class="leasing_status $leasing_sync" style="width:35px;" title="$message">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 32 32" version="1.1"><g id="surface1">
            <path style=" " fill="$color" d="M 16 3 C 8.832031 3 3 8.832031 3 16 C 3 23.167969 8.832031 29 16 29 C 23.167969 29 29 23.167969 29 16 C 29 8.832031 23.167969 3 16 3 Z M 16 5 C 22.085938 5 27 9.914063 27 16 C 27 22.085938 22.085938 27 16 27 C 9.914063 27 5 22.085938 5 16 C 5 9.914063 9.914063 5 16 5 Z M 22.28125 11.28125 L 15 18.5625 L 10.71875 14.28125 L 9.28125 15.71875 L 14.28125 20.71875 L 15 21.40625 L 15.71875 20.71875 L 23.71875 12.71875 Z "/>
            </g></svg>
            </div>
