...
if ( ! class_exists( 'acf_icon_select' ) ) :

	class acf_icon_select extends acf_field {
		...
		*/
		...
		function initialize() {
			...
			// ajax
			add_action( 'wp_ajax_acf/fields/icon_select/query', array( $this, 'ajax_query' ) );
			add_action( 'wp_ajax_nopriv_acf/fields/icon_select/query', array( $this, 'ajax_query' ) );
		}
		...
		*/
		...
		function input_admin_enqueue_scripts() {
			...
			wp_register_script( 'acf-icon_select', get_template_directory_uri() . '/includes/acf-custom-field-types/acf-icon_select/acf-icon_select.js' );
			wp_enqueue_script( 'acf-icon_select' );
			wp_enqueue_style( 'acf-icon_select', get_template_directory_uri() . '/includes/acf-custom-field-types/acf-icon_select/acf-icon_select.css', '', '1.0.0' );
		}
		...
		*/
		...
		function render_field( $field ) {
			...
			foreach ( $choices as &$choice ) {
				$path = \Classes\Cache::getAsset( $choice, true );

				if ( file_exists( $path ) ) {
					
					$choice = file_get_contents( $path );
	...
	// initialize
	acf_register_field_type( 'acf_icon_select' );
