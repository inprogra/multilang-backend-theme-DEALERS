...
class ContactController extends Controller {
	...
	public function renderSearch(): string {
        ...
        foreach($results as $key=>$value) {
            $results[$key]->image = (wp_get_attachment_image_url(get_field('image',$value->ID),'thumbnail') && file_get_contents(wp_get_attachment_image_url(get_field('image',$value->ID),'thumbnail')) ? wp_get_attachment_image_url(get_field('image',$value->ID),'thumbnail') : '/app/themes/partners-site_v2/images/noimage.svg');
        }
			...
			foreach ( $showrooms as $showroom ) {
				...
				foreach ( $employeeCategories as $category ) {
					$queryArgs = [
						...
						'posts_per_page' => -1,
						'meta_query'     => [
							['key' => 'category', 'value' => $category->term_id, 'compare' => '='],
						],
					];
