...
class ProductTagsMetaBox {
    ...
    public function get_post_tags($post_id) {
        ...
        foreach ($tags as $tag) {
            $tag = mb_substr($tag, 0, $max_tag_length); 
            $tag_with_quotes = "'" . $tag . "'";
            $delimiter = empty($limited_tags) ? '' : ',';
            $extra_length = mb_strlen($delimiter . $tag_with_quotes);

            if ($current_length + $extra_length > $max_total_length) {
                break;
            }
